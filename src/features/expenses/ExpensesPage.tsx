...
export default function ExpensesPage() {
  const { workbookData, addRecord, deleteRecord } = useWorkbook();
    ...
    <div className="space-y-6">
      ...
      <section className="space-y-4">
        ...
        {sortedExpenses.length > 0 ? (
          <ExpenseList expenses={sortedExpenses} onDelete={setDeleteTarget} />
        ) : (
          <div className="rounded-3xl border border-dashed border-brand-border bg-brand-bg-card p-8 text-center shadow-subtle">
            ...
            <div className="mt-5">
              <button type="button" onClick={() => setIsFormOpen(true)} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-emerald px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-emerald/90">
                <Plus className="h-4 w-4" />
      ...
      <AddExpenseForm
        ...
        onSubmitExpense={(expense) => addRecord('expenses', expense)}
      />
