...
export default function ExpenseForm({ isOpen, mode, defaultCurrency, expense, onClose, onSubmitExpense }: ExpenseFormProps) {
  ...
  }, [defaultCurrency, expense, isOpen, mode, reset]);
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/40 p-4 sm:items-center">
      <div role="dialog" aria-modal="true" aria-labelledby="expense-form-title" className="w-full max-w-2xl rounded-3xl border border-brand-border bg-brand-bg-card p-5 shadow-soft">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 id="expense-form-title" className="text-xl font-semibold text-brand-text">{mode === 'edit' ? 'Edit expense' : 'Add expense'}</h2>
            <p className="mt-1 text-sm text-brand-text-muted">{mode === 'edit' ? 'Update the expense in your workbook.' : 'Capture a workbook expense without leaving the local-first flow.'}</p>
          </div>
          <button type="button" onClick={onClose} className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-brand-border bg-brand-bg text-brand-text-muted transition-colors hover:bg-brand-border/60" aria-label="Close expense form">
            <X className="h-4 w-4" />
          </button>
        </div>
        ...
        <form className="mt-5 space-y-4" onSubmit={submitHandler}>
          ...
          <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:justify-end">
            ...
            <button type="submit" disabled={isSubmitting} className="inline-flex items-center justify-center gap-2 rounded-2xl bg-brand-emerald px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-brand-emerald/90 disabled:cursor-not-allowed disabled:opacity-60">
              {mode === 'edit' ? <Pencil className="h-4 w-4" /> : <PlusCircle className="h-4 w-4" />}
